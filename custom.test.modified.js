...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      const aliceDepositAmount = utils.parseEther('0.10')
      ...
      const receiptEtherBalanceCheck = utils.parseEther('0.08')
      expect(recipientBalance).to.be.equal(receiptEtherBalanceCheck)
      const omniBridgeBalance = await token.balanceOf(omniBridge.address)
      expect(omniBridgeBalance).to.be.equal(0)
      const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
      const tornadoEtherBalanceCheck = utils.parseEther('0.02')
      expect(tornadoPoolBalance).to.be.equal(tornadoEtherBalanceCheck)
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      // Alice deposits 0.13 into tornado pool L1 layer
      ...
      // Bob withdraws all his funds from the shielded pool L2
